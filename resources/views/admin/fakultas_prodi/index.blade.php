...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
        <form method="GET" action="{{ route('admin.fakultas-prodi.index') }}" class="flex flex-col lg:flex-row gap-3 items-center">
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto">
                <button type="submit" class="w-full lg:w-auto px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white font-semibold text-xs rounded-lg shadow-2xs transition">
                    Filter
                </button>

                @if(!empty($search) || ($fakultas_id && $fakultas_id !== 'all') || ($status && $status !== 'all'))
